...
macro_rules! impl_ctn {
    ($tn:ty, $ctn:expr) => (
        impl CTypeName for $tn {
            fn c_type_name() -> String { $ctn.to_string() }
        }
    );
...
macro_rules! args_to_c {
    () => ("");
    ($arg:ident : $arg_ty:ty) =>
        (format!("{} {}", <$arg_ty>::c_type_name(), stringify!($arg)));
    ($a0:ident : $a0t:ty, $($arg:ident : $arg_ty:ty),+ $(,)*) =>
        (format!("{}, {}", args_to_c!($a0 : $a0t), args_to_c!($($arg : $arg_ty),+)));
}
